...
#!/bin/sh
# Verifica si ya hay una instancia de Konsole abierta
KONSOLE_PID=$(pgrep -x konsole)
if [ -n "$KONSOLE_PID" ]; then
    # Si hay una instancia abierta, ejecuta el script en esa ventana
    konsole -p tabtitle='PFSLE' -e /bin/bash "auto-push-script-2.sh"
else
    # Si no hay una instancia abierta, abre una nueva ventana de Konsole
    konsole -p tabtitle='PFSLE' -e /bin/bash "auto-push-script-2.sh"
fi
